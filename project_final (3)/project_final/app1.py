...
def get_gemini_translation(text, target_lang="English"):
    """Strictly translate text using Gemini API without explanations."""
    payload = {
        "contents": [
            {"parts": [{"text": f"Translate this Sanskrit text to {target_lang} without explanation: {text}"}]}
        ]
    }
    ...
    if response.status_code == 200:
        try:
            ...
            return "⚠️ Error parsing response from Gemini API."
    else:
